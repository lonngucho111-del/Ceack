...
// ---- BATCH PATCH SUPPORT ----
NSMutableData* StaticInlineHookSessionStart(char* machoPath, NSString** outSavePath);
NSString* StaticInlineHookPatchInMemory(NSMutableData* macho, uint64_t vaddr, char* patch);
BOOL StaticInlineHookSessionSave(NSMutableData* macho, NSString* savePath);
...
NSMutableData* StaticInlineHookSessionStart(char* machoPath, NSString** outSavePath) {
    NSString* path = [NSBundle.mainBundle.bundlePath stringByAppendingPathComponent:[NSString stringWithUTF8String:machoPath]];
    NSMutableData* macho = load_macho_data(path);
    if (!macho) return nil;
    ...
    for (int i = 0; i < header->ncmds; i++) {
        if (lc->cmd == LC_ENCRYPTION_INFO_64) {
            struct encryption_info_command_64* info = (struct encryption_info_command_64*)lc;
            if (info->cryptid != 0) return nil; // Not decrypted
        }
        lc = (struct load_command *) ((char *)lc + lc->cmdsize);
    }
    ...
    if (outSavePath) *outSavePath = savePath;
    return macho;
}
...
NSString* StaticInlineHookPatchInMemory(NSMutableData* macho, uint64_t vaddr, char* patch) {
    ...
    for (int i = 0; i < header->ncmds; i++) {
        if (lc->cmd == LC_SEGMENT_64) {
            struct segment_command_64 * seg = (struct segment_command_64 *) lc;
            if(strcmp(seg->segname,"__HOOK_TEXT")==0) text_seg = seg;
            if(strcmp(seg->segname,"__HOOK_DATA")==0) data_seg = seg;
        }
        lc = (struct load_command *) ((char *)lc + lc->cmdsize);
    }

    if (!text_seg || !data_seg) return @"HOOK segments not found!";

    uint64_t funcRVA = vaddr & ~(4-1);
    void *funcData = rva2data(header, funcRVA);
    if (!funcData) return @"Invalid offset!";
    ...
    StaticInlineHookBlock* hookBlock = (StaticInlineHookBlock*)dataData;
    StaticInlineHookBlock* hookBlockRVA = NULL;
    for (int i = 0; i < STATIC_HOOK_DATAPAGE_SIZE / sizeof(StaticInlineHookBlock); i++) {
        if (hookBlock[i].hook_vaddr == 0) {
            hookBlock = &hookBlock[i];
            hookBlockRVA = (StaticInlineHookBlock*)(dataRVA + i * sizeof(StaticInlineHookBlock));
            if (i == 0) {
    ...
    if (!hookBlockRVA) return @"Too many hooks!";
...
BOOL StaticInlineHookSessionSave(NSMutableData* macho, NSString* savePath) {
    return [macho writeToFile:savePath atomically:NO];
}
