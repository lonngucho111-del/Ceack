...
//             if (lc->cmd == LC_SEGMENT_64) {
//                 struct segment_command_64 * seg = (struct segment_command_64 *) lc;
//                 if(strcmp(seg->segname,"__131HOOK_TEXT")==0)
//                     text_seg = seg;
//                 if(strcmp(seg->segname,"__131HOOK_DATA")==0)
//                     data_seg = seg;
//             }
...
//     NSString* savePath = [NSString stringWithFormat:@"%@/Documents/HACKVIPIOS/%s", NSHomeDirectory(), machoPath];
...
void* StaticInlineHookFunction2(void* add, uint64_t vaddr, void* replace)
{
    if(!add) {
        return NULL;
    }
    
    StaticInlineHookBlock* hookBlock = find_hook_block(add, vaddr);
    if(!hookBlock) {
        return NULL;
    }
    
    hookBlock->target_replace = replace;
    return (void*)((uint64_t)add + hookBlock->original_vaddr);
}
...
#define InlineHookAddr(lib_start_addr, x, y, z) \
{ \
    void* result = StaticInlineHookFunction2(lib_start_addr, x, (void *) y); \
    *(void **) (&z) = (void*) result; \
}

#define InlineHookEx(lib_name, x, y, z) \
{ \
    void* result = StaticInlineHookFunction(lib_name, x, (void *) y); \
    *(void **) (&z) = (void*) result; \
}

#define InlineHook(x, y, z) \
{ \
    void* result = StaticInlineHookFunction(DEFAULT_MACHO_PATH, x, (void *) y); \
    *(void **) (&z) = (void*) result; \
}
...
#define ActiveOff2(libname, vaddr, patch) \
    ActiveCodePatch(libname, vaddr, patch)

#define Patch(vaddr, patch) \
    StaticInlineHookPatch(DEFAULT_MACHO_PATH, vaddr, patch)
    
#define Patch2(libname, vaddr, patch) \
    StaticInlineHookPatch(libname, vaddr, patch)

#define DeActiveOff(vaddr, patch) \
    DeactiveCodePatch(DEFAULT_MACHO_PATH, vaddr, patch)
