...
namespace ImGui
{
    ...
    IMGUI_API ImDrawData*   GetDrawData();                              // 在 Render() 之后有效，直到下一次调用 NewFrame()。这是你必须渲染的。

    // Demo, Debug, Information
    IMGUI_API void          ShowDemoWindow(bool* p_open = NULL);        // 创建演示窗口。演示大多数 ImGui 功能。打电话来了解图书馆！尽量让它在您的应用程序中始终可用！
    ...
    IMGUI_API void          Unindent(float indent_w = 0.0f);                                // 将内容位置向左移动，通过 indent_w 或 style.IndentSpacing 如果 indent_w <= 0
    IMGUI_API void          BeginGroup();                                                   // 锁定水平起始位置
    IMGUI_API void          EndGroup();                                                     // unlock horizontal starting position + capture the whole group bounding box into one "item" (so you can use IsItemHovered() or layout primitives such as SameLine() on whole group, etc.)
    IMGUI_API void   		BeginGroupPanel(const char* name, const ImVec2& size_arg);
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddConvexPolyFilled(const ImVec2* points, int num_points, ImU32 col); //注意：抗锯齿填充要求点按顺时针顺序排列。
    IMGUI_API void  AddBezierCubic(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImU32 col, float thickness, int num_segments = 0); //三次贝塞尔（4 个控制点）
    ...
    // Advanced
    IMGUI_API void  AddCallback(ImDrawCallback callback, void* callback_data);  //您的渲染函数必须检查 ImDrawCmd 中的“UserCallback”并调用该函数而不是渲染三角形。
...
struct ImFontAtlas
{
    ...
    ImVec4                      TexUvLines[IM_DRAWLIST_TEX_LINES_WIDTH_MAX + 1];  // 烘焙抗锯齿线的 UV
