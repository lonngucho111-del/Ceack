...
#define NSSENCRYPT_NS(Str) @(ENCRYPT(Str)) // A string criptografada será convertida em NSString
#define NSSENCRYPT_CSTR(str) ENCRYPT_NS(str) // Aqui já chamamos a macro ENCRYPT_NS diretamente sem o uso do stringWithUTF8String
#define ENCRYPT_NS(str) @(ENCRYPT(str)) // Convertendo para NSString diretamente usando a macro ENCRYPT

#define ENCRYPT_NS1(str) ENCRYPT_CSTR(str) // Usa a versão com const char* diretamente
#define ENCRYPT_CSTR(str) (ENCRYPT(str)) // Retorna const char* diretamente da criptografia

#define ENCRYPT_CSTR(str) (ENCRYPT(str)) // Retorna const char* diretamente da criptografia
#define DECRYPT_TO_NSString(str) [NSString stringWithUTF8String:ENCRYPT(str)]
#endif
...
#define ENCRYPTHEX(Str) (char*)ENCRYPT(Str)
